...
AI Social Listening Solution - Eva Cosmetics
Hackathon Challenge CH-05
...
3. INPUT DATA:
   ...
   - social_listening_structure (1).xlsx
...
2. Upload social_listening_structure (1).xlsx when prompted
...
The pipeline generates 2 output files:
...
✓ Sentiment Analysis: 58.57% accuracy (validated)
...
FIX: Check the internet connection. Models download automatically on first run.

ISSUE: Out of memory error
FIX: Use Google Colab with GPU enabled or reduce batch size in code.

ISSUE: Sentiment accuracy lower than expected
FIX: This is normal. 58% is good for a simple pipeline. Can be improved with
...
Sentiment Accuracy: 58.57% (validated against ground truth)
